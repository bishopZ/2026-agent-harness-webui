...
In Review (approval queue): [count]
  - [Idea] — source: leftover markdown table | artifact detector — reviewDocumentPath: initiatives/.../0N_....md
```
...
- **If not in `priorities.json`:** add an entry. Use this logic to set **lifecycle**:
  ...
  **Waiting-state override (do this before accepting the stage default):**
  ...
  1. Prefer an **In Review row from a leftover markdown idea table** for this idea when step 4c found one — that wins over filename mapping.
...
### 4c - Handle leftover markdown idea tables (In Review, Done, Dropped)

If the imported material still has a leftover markdown idea table, treat it as a **temporary source of truth** for queue and history — apply it **before** discarding or ignoring the file.
...
3. **Do not discard the table until** In Review, Done, and Dropped rows have been applied to `priorities.json` (or the user explicitly confirms skipping specific rows). Leave deletion to a later cleanup or health-check pass after registry verification — never delete the queue source mid-import.

If the import still has leftover markdown registries at the harness root, mention `npm run migrate-registry` once after structural import (see [`docs/priorities-registry.md`](../../docs/priorities-registry.md)).
...
- Registry is `priorities.json` only. If a leftover markdown idea table is present, migrate its In Review / Done / Dropped rows into `priorities.json` and history files first (step 4c), then leave deletion to a verified cleanup pass.
- Never discard a leftover markdown idea table mid-import before In Review rows are applied to `priorities.json`.
