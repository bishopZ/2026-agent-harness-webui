...
3. **Approval queue:** For each idea with `lifecycle: "In Review"`, confirm the idea key matches the folder, artifact `**Status:** In Review` (when a brief/stage file exists), and `notes` align. Confirm a valid `reviewDocumentPath:` line when the Web UI should deep-link (see `AGENTS.md`). **Lifecycle** is usually **`In Review`** for a “stage done — approve to advance” gate; it can be **`On Hold`** if the entry is for visibility (dependency block after a draft artifact) and **notes** say so — those ideas will **not** appear in the Web UI approval table unless `lifecycle` is still `In Review`. Scan for ideas whose artifacts say **In Review** but `priorities.json` does not (fix priorities, or document an exception in **notes**). **Gap:** that scan does **not** find **`On Hold`** (or other non–**In Review**) ideas with a written brief and no approval-queue presence — set those to `In Review` by hand when you want them on the Web UI queue, or add a one-line **notes** reminder.
