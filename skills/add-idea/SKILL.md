...
3. **Idea title** - short, noun-style name. It must match the `projects/[Project]/[Idea]/` folder name when lifecycle artifacts exist.
...
6. **Richness** - one line only, or extra detail. Rich content goes in `projects/[Project]/[Idea]/01_brief.md`.
...
| 2 | If the **project is new**, add the project in `priorities.json` (`priority`, `purpose`, empty `ideas`) and create `initiatives/[Initiative]/projects/[Project Name]/` with `00-how-to-use.md`. |
| 3 | Create `initiatives/[Initiative]/projects/[Project Name]/[Idea Name]/` when adding artifacts or a brief. |
| 4 | If there is **rich content**, create `01_brief.md` with `**Status:** In Review` when awaiting approval. Set `lifecycle: "In Review"`, `lastUpdated` to today, and `notes` with next step + brief link. |
...
Create only when `projects/[Project Name]/` is new:

- State that idea work lives under `projects/[Project Name]/[Idea Name]/`.
- Link to `docs/priorities-registry.md`, `SYSTEM_OVERVIEW.md`, and `IDEA_LIFECYCLE.md` (adjust `../` depth from file location).
